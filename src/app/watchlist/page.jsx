...
function UpgradeModal({ onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-white rounded-2xl border border-slate-200 shadow-[0_24px_60px_rgba(15,23,42,0.18)] w-full max-w-md p-6 sm:p-8 fade-up max-h-[90vh] overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
        {/* Close */}
...
function StockDetail({ stock, onRemove }) {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="px-4 sm:px-8 py-5 sm:py-6 border-b border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:justify-between mb-5">
          ...
          {/* Cross-nav + remove */}
          <div className="flex flex-wrap items-center gap-1.5 justify-end shrink-0 ml-2">
            <Link href="/screener"  className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-slate-500 hover:text-[#1e3a8a] border border-slate-200 hover:border-slate-300 bg-white rounded-lg transition-all duration-150 group whitespace-nowrap">
              <Search    className="w-3 h-3 group-hover:text-[#2563eb] shrink-0" />
              <span className="hidden sm:inline">Screener</span>
            </Link>
            <Link href="/analysis"  className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-slate-500 hover:text-[#1e3a8a] border border-slate-200 hover:border-slate-300 bg-white rounded-lg transition-all duration-150 group whitespace-nowrap">
              <BarChart3 className="w-3 h-3 group-hover:text-[#2563eb] shrink-0" />
              <span className="hidden sm:inline">Analysis</span>
            </Link>
            <Link href="/portfolio" className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-slate-500 hover:text-[#1e3a8a] border border-slate-200 hover:border-slate-300 bg-white rounded-lg transition-all duration-150 group whitespace-nowrap">
              <LayoutGrid className="w-3 h-3 group-hover:text-[#2563eb] shrink-0" />
              <span className="hidden sm:inline">Portfolio</span>
            </Link>
            <button onClick={onRemove} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:border-red-200 hover:text-red-500 hover:bg-red-50 transition-all duration-150 shrink-0">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div className="border-b border-slate-100 px-4 sm:px-8 flex gap-0 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
      ...
      {/* Body */}
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-5 sm:py-6 space-y-6" style={{ scrollbarWidth: 'thin' }}>
...
function WatchlistTab({ list, isActive, onClick, onRename, onDelete, canDelete }) {
  ...
  return (
    <div
      className={`group relative flex items-center gap-1.5 px-2 sm:px-3 py-2 rounded-lg cursor-pointer transition-all duration-150 min-w-0 flex-1 sm:flex-none ${
    ...
    >
      <Star className={`w-3 h-3 shrink-0 ${isActive ? 'text-[#2563eb]' : 'text-slate-300'}`} />
      ...
      {editing ? (
        <input
          ...
          className="flex-1 text-xs sm:text-sm font-medium bg-transparent outline-none min-w-0 w-16 sm:w-24 border-b border-[#2563eb]"
        />
      ) : (
        <span className="text-xs sm:text-sm font-medium truncate max-w-[70px] sm:max-w-[120px]">{list.name}</span>
      )}
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-8 py-10 space-y-10">
          ...
          <div className="fade-up delay-3 grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6 items-start">
            ...
            <div className="border border-slate-200 rounded-xl bg-white overflow-hidden">
              ...
              <div className="px-3 sm:px-4 pt-4 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-1.5 bg-slate-50 rounded-xl p-1.5 min-w-0">
                  ...
                  {/* New / Upgrade — always icon-only, no text ever */}
                  <button
                    onClick={handleCreateList}
                    title={watchlists.length >= MAX_FREE_WATCHLISTS ? 'Upgrade to Pro' : 'New watchlist'}
                ...
                <div className="flex items-center justify-between mt-3 gap-2">
                  <p className="text-[11px] text-slate-400 shrink-0">
                    <span className="font-semibold text-slate-600">{activeStocks.length}</span> stocks
                  </p>
          ...
          {watchlists.length >= MAX_FREE_WATCHLISTS && (
            <div className="fade-up delay-4">
              <div className="border border-amber-200 bg-amber-50 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-amber-600 mt-0.5 leading-relaxed">You're using {watchlists.length} of {MAX_FREE_WATCHLISTS} watchlists. Upgrade to Pro for unlimited watchlists and advanced features.</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowUpgrade(true)}
                  className="self-start sm:self-auto shrink-0 flex items-center gap-2 px-4 py-2.5 bg-[#1e3a8a] hover:bg-[#1e40af] text-white text-xs font-semibold rounded-lg transition-all duration-150 shadow-sm"
                >
